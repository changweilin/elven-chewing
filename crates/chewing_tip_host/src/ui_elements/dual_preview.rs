...
impl RenderedView {
    fn new(user_data: *const DualPreview) -> Result<RenderedView, UiError> {
        ...
        let window = create_ime_popup_window(w!("ChewingDualPreviewWindow"), user_data.cast());
        unsafe {
...
impl DualPreview {
    pub(crate) fn window_register_class(hinst: HINSTANCE) {
        register_ime_window_class(hinst, w!("ChewingDualPreviewWindow"), Some(wnd_proc));
    }
